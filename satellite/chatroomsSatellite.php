                            ...
                            if ($lfdu_RSLT['status'] != "RESTRICTED") {
                                ...
                                if ($serverconfig['require_email'] == true) {
                                    ...
                                    if ($lfdu_RSLT['2fa_admission'] == "1") {
                                        ...
                                        $conn->send('{"status":"fail", "error":"email_not_set","update_link":"' . $serverconfig["email_activation_url"] . '"}');
                                        $this->clients->detach($conn);
                                        $conn->close();
                                    }
                                } else {
                            ...
                            if ($greenlight == true) {
                                ...
                                if (!is_bool($lfdc)) {
                                    ...
                                    while ($row = mysqli_fetch_assoc($lfdc)) {
                                        ...
                                        $returnjson[$rewind] = array("id" => $actualid, "author" => $author, "channel" => $channl, "content" => $contnt, "date" => $datumo, "username" => $authoruname, "attachment1" => $attach1);
                                        $rewind++;
                                ...
                                if (mysqli_num_rows($security_lfdu) != 0) {
                                    ...
                                    if (!is_bool($lfdu)) {
                                        ...
                                        if (mysqli_num_rows($lfdu) != 0) {
                                            ...
                                            $cdate = /*stripslashes(htmlspecialchars(gmdate("F nS Y, G:i", */
                                            $lfdu_RSLT['creationdate'];
                                            $userroles = $lfdu_RSLT['roles'];
            ...
            case "editprofile:password":
                if (!empty($dataset['authentication']) and !empty($dataset['old_password']) and !empty($dataset['password'])) {
                    ...
                    if (mysqli_num_rows($lfdu) > 0 and password_verify($password, $strings['password'])) {
                        ...
                        $lfdpfp = mysqli_query($ctds, "UPDATE `accounts` SET `password`='" . password_hash($dataset['password'], PASSWORD_DEFAULT) . "',`authentication`='" . $new_token . "' WHERE `authentication`='" . $auth . "'");
...
$webSock = new React\Socket\SecureServer($webSock, $loop, ['local_cert' => $config['cert_path'], 'local_pk' => $config['pkey_path'], 'allow_self_signed' => !$config['force_secure'], 'verify_peer' => FALSE]);
$webServer = new Ratchet\Server\IoServer(new Ratchet\Http\HttpServer(new Ratchet\WebSocket\WsServer(new Chatroom())), $webSock, $loop);
$webServer->run();
?>
